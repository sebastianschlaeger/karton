...
def process_orders(orders_data):
    ...
    for order in orders_data:
        ...
        )  # This is now in grams

        processed_order = {
            'order_number': order.get('OrderNumber', 'Unknown'),
            'created_at': order.get('CreatedAt'),
            'total_weight': total_weight,  # Store weight in grams
            'products': [
                {
        ...
        if has_special_sku:
            ...
            allocated_box, _ = allocate_box(int(total_weight))  # No need to convert, already in grams
            allocation_reason = f"Weight: {total_weight:.0f} g"
